...
2、简单的差分放大器双端输出时，对 ________ 信号有较强的抑制能力，而对 ________ 信号有较强的放大作用。

> 共模      差模

3、已知理想差分放大电路输入端电压 ${v_{i1}} = 100{\rm{ \mu V}}$，${v_{i2}} = 80{\rm{ \mu V}}$，则其差模输入电压 

${v_{id}} =$  __________  μV；共模输入电压 ${v_{ic}} =$  __________  μV。

> 20      90。差分为两电压值之差；共模为两电压之平均值。

4、差分放大电路的主要特点是 ________ 。
...
5、差分放大电路的共模抑制比 CMRR 越大，表明电路 ________ 。
...
> D。
>
> 放大电路的零漂（Zero Drift）是指在没有输入信号时，放大器的输出偏离零点（或基准电平）的现象。这种现象通常是由于电路中的不稳定因素引起的，比如温度变化、电源电压波动、电子元件的老化、内部偏置电流的变化等。零漂对于精密放大电路来说是一个重要的性能指标，因为它会影响电路的精确度和稳定性。此处通过差分放大电路来减少对电源波动的敏感性。
...
>
> ​		$\frac{{{v_o}}}{{{v_i}}} = \frac{1}{2}{g_m}{R_C} = \frac{1}{2} \times 9.5 \times 10 = 47.5$  【仅利用了一半电路作为输出，增益减半】
>
...
>
> 【注】也可以不画交流等效电路，用半电路分析

4、如下图所示电路，所有元件理想对称，晶体管的 $\beta$ 均为 100，$r_{bb'}=100{\rm \Omega}$ ，$U_{BEQ} \approx 0.7{\rm V}$ ，$V_T=25{\rm mV}$ 。试计算电位器 $R_W$ 滑动端在中点时 $T_1$ 管和 $T_2$ 管的发射极静态电流 $I_{EQ}$ ，以及放大器增益 $A_d=u_o/u_i$ 和差分输入电阻 $R_i$ 。
...
> 当 $V_{OQ}=0$ 时，$I_{CQ3}·R_{C3}=V_{CC}$，$\therefore I_{CQ3}=\frac{V_{CC}}{R_{C3}}=\frac{12}{12}=1{\rm mA}$ 
>
...
>
> ​	  $I_{R_{C2}}=I_{CQ2}-I_{BQ3}=0.12-0.01=0.11{\rm mA}$ 
>
...
> ​	  $G_m=g_m=0.016{\rm S}$ 
>
> ​	  $A_v=G_m \times R_o=2000{\rm V/V} \approx 66{\rm dB}$ 
>
...
>
> 【注】把达林顿管看作一个 $\beta = \beta_1 \times \beta_2$ 的管子即可
...
> $T_3$ 和 $T_4$       差分      $T_8$       共源      $T_1$、$T_2$ 和 $T_7$      $T_5$ 和 $T_6$        有源负载
